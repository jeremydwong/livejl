...
function mean_colors(image)
	r = [image[i,j].r for i=1:size(image,1),j=1:size(image,2)]
	g = [image[i,j].g for i=1:size(image,1),j=1:size(image,2)]
	b = [image[i,j].b for i=1:size(image,1),j=1:size(image,2)]
	return (mean(mean(r)),mean(mean(g)),mean(mean(b)))
end
...
begin
	function quantize(x::Number)
		...
		return floor(x*10)/10.0
	end
	
	function quantize(c::AbstractRGB)
		# you will write me in a later exercise!
		return RGB(quantize(c.r), quantize(c.g), quantize(c.b))
	end
	
	function quantize(image::AbstractMatrix)
		mat = image
		[mat[i,j] = quantize(mat[i,j]) for i in 1:size(mat,1),j in 1:size(mat,2)]
		return mat
	end
end
...
function invert(c::AbstractRGB)
	
	return RGB(1 - c.r, 1 - c.g, 1 - c.b)
end
...
begin
	function myclamp(x::Number,therange)
		x = x<= therange[2] ? x : therange[2]
		return x >= therange[1] ? x : therange[1]
	end
	
	function noisify(x::Number, s)
		return myclamp(x + (2*rand(1)[1] -1.) * s, [0.,1.])
	end
	
	function noisify(c::AbstractRGB, s)
		# you will write me in a later exercise!
		return RGB(noisify(c.r,s), noisify(c.g,s), noisify(c.b,s))
	end
	
	function noisify(image::AbstractMatrix, s)
		return [noisify(image[i,j],s) for i in 1:size(image,1),j in 1:size(image,2)]
	end
end
...
philip_inverted = [invert(philip[i,j]) for i in 1:size(philip,1),j in 1:size(philip,2)]
...
function blur_1D(v, l::Int64)
	if l < 0
		println("sorry! l must be >=0.")
		return 0
	end
	...
	curb = zeros(2*l+1) 
	for c in 1:length(v)
		[curb[k-c+l+1] = extend(v,k) for k in (c-l):(c+l)]
		vec[c] = mean(curb)
	end
...
# ╔═╡ b942c536-b248-4e91-8ae4-253962181572
# begin 
# 	c = 1
# 	curb = zeros(2*3+1)
# 	[curb[1] = extend(v,k) for k in (c-k):(c+k)]
# end 
@bind lValue Slider(0:1:length(v), show_value = true)

# ╔═╡ b8ee864b-743d-432f-8f3f-6ae5e61b74a2
colored_line(blur_1D(v,lValue))
